    ...
    #verificar si son iguales los cbu o alias
        ...
        if all((cbuOrigen in tupla, cbuDestino in tupla) for tupla in cuentas):
            ...
            try:
                ...
                self.transaccionesMesOrigen = sum(monto[0] for monto in transacciones)
            except Error as e:
                print(e)
            finally:
                cursor.close()
...
# ------------------------------------------------------------------------------------------
    # OBTENER TRANSACCIONES MENSUALES DE LOS INVOLUCRADOS cuenta destino
            try:
                ...
                self.transaccionesMesDestino = sum(monto[0] for monto in transaccionesd)
            except Error as e:
                print(e)
            finally:
                cursor.close()
    ...
    # ------------------------------------------------------------------------------------------
            ...
            try:
                ...
                print("tipo de destino ",tipoDestino)
            except Error as e:
                print(e)
            finally:
                cursor.close()
...
#---------------------RESTA EL MONTO DEL BALANCE DEL ORIGEN-------------------------
            try:
                ...
                cursor = conn.cursor()
                consulta = "UPDATE cuentas SET balance = %s WHERE cbuOrigen=%s OR alias=%s"
...
#------------------------------sumar el balance al destino----------------------
            try:
                nuevobalance = saldoDestino + monto
                db_config = leer_configuracion_db()
                conn = MySQLConnection(**db_config)
                cursor = conn.cursor()
                consulta = "UPDATE cuentas SET balance = %s WHERE cbuOrigen=%s OR alias=%s"
                ...
                cursor.execute(consulta, datos)
                conn.commit()
                QMessageBox.information(None, "Éxito", "Se recibio la transferencia")
            except Error as e:
                print(e)
            finally:
                cursor.close()
